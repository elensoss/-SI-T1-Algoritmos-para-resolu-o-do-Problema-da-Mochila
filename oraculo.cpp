...
int main() {
    std::ifstream file("entrada_1000.txt");
    if (!file.is_open()) {
        std::cerr << "Erro ao abrir entrada_200.txt" << std::endl;
        return 1;
    }
    ...
    for (int i = 0; i < n; ++i) {
        file >> w[i] >> p[i];  // peso e valor
    }
    file.close();

    int resultado = minknap(n, p.data(), w.data(), x.data(), capacidade);
    
    std::cout << "Valor máximo: " << resultado << std::endl;
